<template>
    <div class="relative">
      <div v-for="user in aboutMeInfos.userData" :key="user.id">
         <single-info-box :text="user.text" :isHR="user.isHR" :avatar="user.avatar" :avatar_alternative="user.avatar_alternative" />
      </div>
    </div>
  </template>
  
  <script setup>
  const aboutMeInfos = reactive({
    userData: [
      {
        id: 1,
        text: "Hi, How are you?",
